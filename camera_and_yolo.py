...
# Źródło: ros2 run tf2_ros tf2_echo camera_depth_optical_frame camera_color_optical_frame
...
class ToolPerceptionLiveNode(Node):
    ...
    def _image_callback(self, rgb_msg: Image, depth_msg: Image):
        ...
        for det_id, box in enumerate(results.boxes):
            ...
            cx_2d = (x1 + x2) // 2
            cy_2d = (y1 + y2) // 2
            label = f"{cls_name} {conf:.2f} | Z={z:.3f}m"
            ...
            marker_arr.markers.append(
                self._build_marker(rgb_msg, det_id, cls_name, x_3d, y_3d, z)
            )
            all_points.extend(roi_pts)

            self.get_logger().info(
                f"[{cls_name}] conf={conf:.2f} | "
                f"3D x={x_3d:.3f} y={y_3d:.3f} z={z:.3f}",
    ...
    def _build_marker(self, rgb_msg, det_id, cls_name, x, y, z) -> Marker:
        m = Marker()
